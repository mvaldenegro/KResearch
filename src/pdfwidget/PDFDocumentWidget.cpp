...
void PDFDocumentWidget::mouseReleaseEvent(QMouseEvent *)
{
    ...
    if (!rubberBand->size().isEmpty()) {
        ...
        QRectF rect = QRectF(label->mapFrom(this, rubberBand->pos()), rubberBand->size());
...
QSize PDFDocumentWidget::sizeHint() const
{
    QSize base = QScrollArea::sizeHint();
...
void PDFDocumentWidget::nextPage()
{
...
void PDFDocumentWidget::prevPage()
{
    int page = currentPage - 1;
